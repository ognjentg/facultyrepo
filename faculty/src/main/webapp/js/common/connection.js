...
// common connection methods
    //TODO SESSION EXPIRED ONE MESSAGE
var connection = {
        ...
        fetchSettings: function (viewId, name) {
            util.preloader.inc();
            var promise = webix.promise.defer();

            webix.ajax().get(urlCompanyPath.setting+"/byViewId/" + name, {
                error: function (text, data, xhr) {
                    if (xhr.status == 401) {
                        if (connection.showSEM) {
                            util.messages.showSessionExpiredError();
                    ...
                    if (data.length > 0) {
                        try {
                            ...
                            for (i = 0; i < data.length; i++) {
                                settings[data[i].keySetting] = data[i].valueSetting;
                            }
                        } catch (ex) {
                        }
                        promise.resolve(true);
                    ...
                    if (data.length > 0) {
                        try {
                            if (!viewId.settings)
                                viewId.settings = {};
                            for (i = 0; i < data.length; i++) {
                                viewId.settings[data[i].keySetting] = data[i].valueSetting;
                            }
                        } catch (ex) {
                        }
                    } else {
        ...
        fetchOneSetting: function (viewId, name) {
            ...
            webix.ajax().get(urlCompanyPath.setting+"/"+ name, {
                error: function (text, data, xhr) {
                    if (xhr.status == 401) {
                        if (connection.showSEM) {
                            util.messages.showSessionExpiredError();
                ...
                success: function (text, data, xhr) {
                    var data = data.json();
                    if (util.isset(data.keySetting)) {
                        try {
                            var settings = viewId.settings;
                            settings[data.keySetting] = data.valueSetting;
                        } catch (ex) {
                        }
                        promise.resolve(true);
                    } else {
                        util.messages.showErrorMessage("Nije moguće isparsirati podešavanje: " + name);
                        promise.resolve(false);
                    }
                    util.preloader.dec();
                }
        ...
        fetchPromoType: function (viewId, type) {
            util.preloader.inc();
            var promise = webix.promise.defer();
            webix.ajax().get("hub/promo-item/promo-type/" + type, {
                error: function (text, data, xhr) {
                    if (xhr.status == 401) {
                        if (connection.showSEM) {
                            util.messages.showSessionExpiredError();
                        ...
                        util.messages.showErrorMessage("Nije moguće prikupiti tip promocije.");
                    }
                    util.preloader.dec();
                    promise.reject(false);
                },
                success: function (text, data, xhr) {
                    ...
                    if (data != null) {
                        try {
                            viewId.promoType = data;
                        } catch (ex) {
                        }
                    } else util.messages.showErrorMessage("Nije moguće parsirati tip promocije.");
                    util.preloader.dec();
                    promise.resolve(true);
                }
            });
